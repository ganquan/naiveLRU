...
// 最快的查找必然是哈希表，但是哈希表的增、删、改性能并不是最好的
// 而增、删、改性能最好的是双向链表
// 所以LRUCache的底层数据结构就是哈希表和双链表，利用这两个数据结构的优点解决两个问题
// LRU的算法实现考察，其实是一个很好的综合考察题目
// 对算法、和基础数据结构都覆盖到了
...
// 如果利用defer { lock.unlock() }的方法实现锁操作成对处理，但是函数结尾的clean()就会出现死锁
